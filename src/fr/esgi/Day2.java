...
public class Day2 {
    public static void main(String args[])
    {
        ...
        int[] diffArray = new int[rows];

        int result = 0;

        for(int i = 0; i < rows; i++){ // i => actual row
            Integer min = null;
            Integer max = null;
            for(int j = i*columns; j < (i*columns)+columns; j++){ // j => actual column
                int actualValue = Integer.parseInt(inputArray[j]);
