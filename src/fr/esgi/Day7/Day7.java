...
//				"cntj (57)";
		...
		for (String s : input.split("\n")) {
			...
			if (matcher.find()) {
				...
				nodeMap.put(matcher.group(1), newNode);

				if (matcher.group(3) != null) {
					fatherMap.put(matcher.group(1), matcher.group(3));
				}
			}
		}

		for (Map.Entry<String, String> entry : fatherMap.entrySet()) {
			for (String childName : entry.getValue().split(",\\s+")) {
				nodeMap.get(entry.getKey()).addChild(nodeMap.get(childName));
				nodeMap.get(childName).setFather(nodeMap.get(entry.getKey()));
			}
		}
